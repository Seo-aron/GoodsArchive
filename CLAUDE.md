오늘 우리가 함께 기획한 4개의 핵심 탭(컬렉션, 전시장, 기록, 내정보) 구조와 수집가들의 심리를 저격할 '기록(데이터 요약)' 기능을 `CLAUDE.md` 파일에 완벽하게 반영했습니다!

기존에 잘 작성된 백엔드 세팅과 DB 설계 등은 그대로 보존하고, 프론트엔드 UI 설계 부분(Step 6)과 요약 데이터를 내려줄 백엔드 API 명세 부분(Step 4)만 깔끔하게 업데이트했습니다.

아래 마크다운 전체를 복사해서 기존 `CLAUDE.md` 파일에 그대로 덮어씌워 주시면 됩니다.
...
* `GET  /api/goods/summary` — **[신규]** 내 수집 기록 요약 조회 (총 개수, 총 자산 가치 누적 금액 등)
...
### 🔲 Step 6 — Flutter UI 본격 구현 (4대 핵심 탭 구조)

앱 하단 탭바(`BottomNavigationBar`)를 기반으로 4개의 독립된 메인 화면을 분리 및 구현:

1. **컬렉션 (Collection):** 사진을 찍어 누끼를 따고 굿즈를 등록(우측 하단 + 버튼)하며, 내 굿즈 목록을 리스트 형태로 조회하는 화면. (`screens/collection_screen.dart`)
2. **전시장 (Showcase):** 컬렉션에 등록된 굿즈들을 불러와 2D 배경 위에 자유롭게 줌인/아웃 및 드래그하여 꾸미고 배치 좌표를 저장하는 화면. (`screens/showcase_screen.dart`)
3. **기록 (Record):** 굿즈 데이터를 바탕으로 총 수집품 개수와 총 자산 가치 등을 수치화/데이터화하여 요약해서 보여주는 대시보드 화면. (`screens/record_screen.dart`)
4. **내정보 (My Info):** 프로필 설정, 공지사항, 로그아웃 등이 배치되는 화면. (`screens/my_info_screen.dart`)

* **공통:** JWT 토큰 안전 저장 (`flutter_secure_storage` 등 활용) 및 각 화면별 Spring Boot API 통신 연동.
